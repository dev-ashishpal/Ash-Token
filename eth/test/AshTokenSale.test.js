...
contract("AshTokenSale", (accounts) => {
	...
	it("facilitates token buying", async () => {
		...
		assert.equal(receipt.logs.length, 1, 'triggers one event');
	  	...
	  	assert.equal(receipt.logs[0].args._buyer, buyer, 'logs the account the tokens are authorized by');
	  	assert.equal(receipt.logs[0].args._amount, numberOfTokens, 'logs the account the tokens are authorized to');
		...
		const senderBalance = await tokenInstance.balances(tokenSaleInstance.address);
		...
		try{
			await tokenSaleInstance.buyTokens(80000	, {from: buyer, value: 1});
			assert(false);
		} catch(err) {
			assert(err);
		}
	...
	it("ending token sale", async () => {
		...
		assert.equal(balance.toNumber(), 999990, 'returns all unsold daap token to admin');
		// const price = await tokenSaleInstance.tokenPrice();
